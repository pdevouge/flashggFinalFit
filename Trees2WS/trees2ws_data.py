...
def get_options():
  ...
  parser.add_option('--applyMassCut',dest='applyMassCut', default=True, action="store_true", help='Apply cut on CMS_hgg_mass')
  parser.add_option('--massCutRange',dest='massCutRange', default='130,300', help='CMS_hgg_mass cut range')
...
print(f"Low cut: {massLow}, high cut: {massHigh}")
...
def add_vars_to_workspace(_ws=None,_dataVars=None,_minMass='100',_maxMass='5000', _midMass='500'):
  # Add intLumi var
...
varNames = add_vars_to_workspace(ws,dataVars, massLow, massHigh, massMid)
...
for cat in cats:
  ...
  for ev in t:
    mass_val = getattr(ev, "CMS_hgg_mass")
    if opt.applyMassCut:
      if mass_val < massLow or mass_val > massHigh:
        continue
    for var in dataVars:
