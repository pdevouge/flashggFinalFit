...
# MHLow, MHHigh = '120', '130' # REMOVED HARD CODED VALUES
...
def get_options():
  ...
  parser.add_option('--width', dest='width', default='001', help="Width point to fit")
...
kappa = 0.01
sigma = math.sqrt(2)*kappa**2*int(opt.mass)
MHLow, MHHigh = int(opt.mass)-0.1*int(opt.mass), int(opt.mass)+0.1*int(opt.mass)#(int(opt.mass)-30*int(int(opt.mass)/100),int(opt.mass)+30*int(int(opt.mass)/100))
xvar.setRange(MHLow, MHHigh)
xvarFit = xvar.Clone()
dZ = inputWS0.var("dZ")
...
for proc in opt.procs.split(","):
  WSFileName = glob.glob("%s/output*M%s_kMpl%s*%s.root"%(opt.inputWSDir,opt.mass,opt.width,proc))[0]
  f = ROOT.TFile(WSFileName,"read")
  inputWS = f.Get(inputWSName__)
  d = reduceDataset(inputWS.data("%s_%s_%s_%s_%s"%(procToData(proc.split("_")[0]),opt.mass,opt.width,sqrts__,opt.cat)),aset)
...
for pidx, proc in enumerate(procsToFTest):
  ...
  if datasets_RV[opt.mass].numEntries() < opt.threshold: continue
    ...
    for nGauss in range(1,opt.nGaussMax+1):
      k = "nGauss_%g"%nGauss
      print("------> Starting Simultaneous Fit for RV")
      ssf = SimultaneousFit("fTest_RV_%g"%nGauss,proc,opt.cat,datasets_RV,xvar.Clone(),MH,MHLow,MHHigh,opt.mass,opt.mass,opt.nBins,0,opt.minimizerMethod,opt.minimizerTolerance,verbose=True)
