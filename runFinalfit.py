...
backgroundScriptCfg = {{
  ...
  # Setup
  'inputWS': '{input_dir}/data/ws/allData_PreEE.root',  # Input RooWorkspace
...
cmd = f"python3 RunWSScripts.py --inputDir {input_dir}/signal/ --inputConfig config_high_mass.py \
    --year {year} --mode trees2ws --batch local --modeOpts \"--minMass {MLow} --maxMass {MHigh}\""
...
cmd = f"""python3 RunPackager.py --cats {cat} --exts {ext} \
...
cmd = f"""python3 RunYields.py --inputWSDirMap {year}={input_dir}/signal/ws_{cfg["signal"]["procs"]} \
    --cats {cat} --procs {cfg["signal"]["procs"]} --ext {ext} --skipCOWCorr --batch local --mass 700"""
subprocess.call(cmd, shell=True)

cmd = f"""python3 makeDatacard.py --ext {ext} --years {year} --prune --doTrueYield \
