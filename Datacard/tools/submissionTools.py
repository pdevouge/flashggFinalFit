...
def writeSubFiles(_opts):
  ...
  if _opts['batch'] == "condor":
    ...
    for cidx in range(_opts['nCats']):
      ...
      _f.write("  python3 %s/makeYields.py --cat %s --procs %s --ext %s --width %s --mass %s --inputWSDirMap %s %s\n"%(dwd__,c,_opts['procs'],_opts['ext'],_opts['width'],_opts['mass'],_opts['inputWSDirMap'],_opts['modeOpts']))
      _f.write("fi\n")
  ...
  if (_opts['batch'] == "IC")|(_opts['batch'] == "SGE")|(_opts['batch'] == "local" ):
    ...
    for cidx in range(_opts['nCats']):
      ...
      writePreamble(_f)
      _f.write("python3 %s/makeYields.py --cat %s --procs %s --ext %s --width %s --mass %s --inputWSDirMap %s --sigModelWSDir %s --sigModelExt %s --bkgModelWSDir %s --bkgModelExt %s %s\n"%(dwd__,c,_opts['procs'],_opts['ext'],_opts['width'],_opts['mass'],_opts['inputWSDirMap'],_opts['sigModelWSDir'],_opts['sigModelExt'],_opts['bkgModelWSDir'],_opts['bkgModelExt'],_opts['modeOpts']))
      _f.close()
